...
module PxModule
  class PerimeterxActivitiesClient < PerimeterxRiskClient
    ...
    def send_to_perimeterx(activity_type, px_ctx, details = [])
      ...
      @px_config[:additional_activity_handler].call(activity_type, px_ctx, details) if @px_config.key?(:additional_activity_handler)

      details[:cookie_origin] = px_ctx.context[:cookie_origin] unless px_ctx.context[:px_cookie].empty?
    ...
    def send_block_activity(px_ctx)
      ...
      unless @px_config[:send_block_activities]
      ...
      details[:risk_rtt] = px_ctx.context[:risk_rtt] if px_ctx.context.key?(:risk_rtt)

      details[:px_orig_cookie] = px_ctx.context[:px_orig_cookie] if px_ctx.context.key?(:px_orig_cookie)
    ...
    def send_page_requested_activity(px_ctx)
      ...
      return unless @px_config[:send_page_activities]
      ...
      details[:px_cookie] = px_ctx.context[:decoded_cookie] if px_ctx.context.key?(:decoded_cookie)

      details[:px_orig_cookie] = px_ctx.context[:px_orig_cookie] if px_ctx.context.key?(:px_orig_cookie)

      details[:px_cookie_hmac] = px_ctx.context[:cookie_hmac] if px_ctx.context.key?(:cookie_hmac)
