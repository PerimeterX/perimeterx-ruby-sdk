def validate_hash_schema(hash, schema)
  hash.each do |key, value|
    if schema.key?(key) && value != nil
      ...
      if !schema[key][:types].include?(value.class)
        raise Exception.new("PerimeterX: Type of #{key} should be one of #{schema[key][:types]} but instead is #{value.class}")
      end
      ...
      if value.class == Array
        value.each do |element|
          if !schema[key][:allowed_element_types].include?(element.class)
            raise Exception.new("PerimeterX: #{key} may only contain elements of the following types: #{schema[key][:allowed_element_types]} but includes element of type #{element.class}")
          end
        end
      end
    end
  ...
  schema.each do |key, value|
    if value[:required] && hash[key].nil?
      raise Exception.new("PerimeterX: #{key} configuration is missing")
    end
  end
end
